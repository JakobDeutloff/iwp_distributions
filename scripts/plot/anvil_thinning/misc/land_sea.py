...
path = "/work/bu1562/m301049/diurnal_cycle_dists/"
hists = {
...
temp = {
    "all": xr.open_dataset("/work/bu1562/m301049/era5/monthly/t2m_tropics.nc").t2m,
    "sea": xr.open_dataset("/work/bu1562/m301049/era5/monthly/t2m_tropics_sea.nc").t2m,
...
for key in hists_normalized.keys():
    temp_detrend = xr.DataArray(
        detrend(temp[key]), coords=temp[key].coords, dims=temp[key].dims
    )
    temp_deseason[key] = temp_detrend.groupby("time.month") - temp_detrend.groupby(
        "time.month"
    ).mean("time")
...
cre = cre.interp(iwp=np.log10(hists["all"].iwp), method="linear").drop_vars("iwp")
...
sea_fraction = 0.73
combined_feedback = feedback["sea"] * sea_fraction + feedback["land"] * (
    1 - sea_fraction
)
...
ax.legend()
fig.savefig(
    "plots/anvil_thinning/land_sea/distributions.png", dpi=300, bbox_inches="tight"
)
...
axes[1].scatter(1, combined_feedback[1:].sum(), label="Combined", color="k", marker="x")
