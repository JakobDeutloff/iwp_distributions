...
predictors["max_d"] = xr.open_dataarray(
    "/work/bu1562/m301049/era5/monthly/max_convergence_60_95hPa.nc",
...
predictors["t_surf"] = xr.open_dataarray(
    "/work/bu1562/m301049/era5/monthly/t2m_tropics.nc", decode_timedelta=False
)
...
max_d_temp = linregress(
    predictors_detrend["t_surf"].values, predictors_detrend["max_d"].values
)
stab_temp = linregress(
    predictors_detrend["t_surf"].values, predictors_detrend["stability"].values
)
max_d_stab = linregress(
    predictors_detrend["stability"].values, predictors_detrend["max_d"].values
)
...
ax.plot(
    slopes["t_surf"].sel(iwp=slice(1e-3, None)).iwp,
    slopes["t_surf"].sel(iwp=slice(1e-3, None)),
    label="$\partial f / \partial T$",
    color="k",
)
ax.plot(
    slopes_temp["max_d"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp["max_d"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial D \cdot \partial D / \partial T$",
    color="r",
)
ax.plot(
    slopes_temp["stability"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp["stability"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial S \cdot \partial S / \partial T$",
    color="b",
)
ax.plot(
    slopes_temp["max_d_stab"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp["max_d_stab"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial D \cdot \partial D / \partial S \cdot \partial S / \partial T$",
    color="g",
)
ax.set_xscale("log")
...
fig.savefig(
    "plots/anvil_thinning/iris/slopes_monthly.png", dpi=300, bbox_inches="tight"
)

# %% make scatterplot of predictors and slopes
fig, axes = plt.subplots(1, 3, figsize=(12, 4))
axes[0].scatter(
    predictors_detrend["t_surf"].values,
    predictors_detrend["stability"].values,
    color="b",
    alpha=0.5,
)
...
axes[1].scatter(
    predictors_detrend["t_surf"].values,
    predictors_detrend["max_d"].values,
    color="r",
    alpha=0.5,
)
...
axes[1].plot(
    predictors_detrend["t_surf"].values,
    predictors_detrend["t_surf"].values * slopes_predictors["max_d"]
    + intercepts_predictors["max_d"],
    color="k",
)
axes[0].plot(
    predictors_detrend["t_surf"].values,
    predictors_detrend["t_surf"].values * slopes_predictors["stability"]
    + intercepts_predictors["stability"],
    color="k",
)
axes[2].plot(
    predictors_detrend["stability"].values,
    predictors_detrend["stability"].values * slopes_predictors["max_d_stab"]
    + intercepts_predictors["max_d_stab"],
    color="k",
)
...
axes[0].text(
    0.05,
    0.95,
    f"$\partial S / \partial T = ${slopes_predictors['stability']:.2e} mK hPa$^{{-1}}$ K$^{{-1}}$",
...
axes[1].text(
    0.05,
    0.95,
    f"$\partial D / \partial T = ${slopes_predictors['max_d']:.2e} day$^{{-1}}$ K$^{{-1}}$",
...
axes[2].text(
    0.05,
    0.95,
    f"$\partial D / \partial S = ${slopes_predictors['max_d_stab']:.2e} hPa day$^{{-1}}$ mK$^{{-1}}$",
...
fig.savefig(
    "plots/anvil_thinning/iris/predictor_scatter_monthly.png",
    dpi=300,
    bbox_inches="tight",
)
...
predictors_annual = {}
for predictor_name, predictor in predictors.items():
    predictor_annual = predictor.groupby("time.year").mean(
        "time"
    )  # calculate_jj_mean(predictor)
    predictor_annual_detrend = xr.DataArray(
        detrend(predictor_annual),
        coords=predictor_annual.coords,
        dims=predictor_annual.dims,
    )
...
slopes_predictors_annual = {
    "max_d": linregress(
        predictors_annual["t_surf"].values, predictors_annual["max_d"].values
    ).slope,
    "stability": linregress(
        predictors_annual["t_surf"].values, predictors_annual["stability"].values
    ).slope,
    "max_d_stab": linregress(
        predictors_annual["stability"].values, predictors_annual["max_d"].values
    ).slope,
}
...
for predictor_name in ["max_d", "stability"]:
    ...
    err_temp_annual[predictor_name] = (
        err_annual[predictor_name] * slopes_predictors_annual[predictor_name]
    )
slopes_temp_annual["max_d_stab"] = (
    slopes_annual["max_d"]
    * slopes_predictors_annual["max_d_stab"]
    * slopes_predictors_annual["stability"]
)
...
ax.axhline(0, color="k", linewidth=0.5)
ax.plot(
    slopes_annual["t_surf"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_annual["t_surf"].sel(iwp=slice(1e-3, None)),
    label="$\partial f / \partial T$",
    color="k",
)
ax.plot(
    slopes_temp_annual["max_d"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp_annual["max_d"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial D \cdot \partial D / \partial T$",
    color="r",
)
ax.plot(
    slopes_temp_annual["stability"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp_annual["stability"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial S \cdot \partial S / \partial T$",
    color="b",
)
ax.plot(
    slopes_temp_annual["max_d_stab"].sel(iwp=slice(1e-3, None)).iwp,
    slopes_temp_annual["max_d_stab"].sel(iwp=slice(1e-3, None)),
    label="$\partial f/\partial D \cdot \partial D / \partial S \cdot \partial S / \partial T$",
    color="g",
)
ax.set_xscale("log")
...
fig, axes = plt.subplots(1, 3, figsize=(12, 4))
axes[0].scatter(
    predictors_annual["t_surf"].values,
    predictors_annual["stability"].values,
    color="b",
    alpha=0.5,
)
axes[0].set_xlabel("T / K")
...
axes[1].scatter(
    predictors_annual["t_surf"].values,
    predictors_annual["max_d"].values,
    color="r",
    alpha=0.5,
)
axes[1].set_xlabel("T / K")
...
axes[1].plot(
    predictors_annual["t_surf"].values,
    predictors_annual["t_surf"].values * slopes_predictors_annual["max_d"]
    + intercepts_predictors_annual["max_d"],
    color="k",
)
axes[0].plot(
    predictors_annual["t_surf"].values,
    predictors_annual["t_surf"].values * slopes_predictors_annual["stability"]
    + intercepts_predictors_annual["stability"],
    color="k",
)
axes[2].plot(
    predictors_annual["stability"].values,
    predictors_annual["stability"].values * slopes_predictors_annual["max_d_stab"]
    + intercepts_predictors_annual["max_d_stab"],
    color="k",
)
...
axes[0].text(
    0.05,
    0.95,
    f"$\partial S / \partial T = ${slopes_predictors_annual['stability']:.2e} mK hPa$^{{-1}}$ K$^{{-1}}$",
    transform=axes[0].transAxes,
    verticalalignment="top",
)
axes[1].text(
    0.05,
    0.95,
    f"$\partial D / \partial T = ${slopes_predictors_annual['max_d']:.2e} day$^{{-1}}$ K$^{{-1}}$",
    transform=axes[1].transAxes,
    verticalalignment="top",
)
axes[2].text(
    0.05,
    0.95,
    f"$\partial D / \partial S = ${slopes_predictors_annual['max_d_stab']:.2e} hPa day$^{{-1}}$ mK$^{{-1}}$",
    transform=axes[2].transAxes,
    verticalalignment="top",
)
fig.tight_layout()
...
fig.savefig(
    "plots/anvil_thinning/iris/predictor_scatter_annual.png",
    dpi=300,
    bbox_inches="tight",
)
...
for predictor_name in predictors.keys():
    axes[0].plot(
        r[predictor_name].sel(iwp=slice(1e-3, 20)).iwp,
        r[predictor_name].sel(iwp=slice(1e-3, 20)),
        label=labels[predictor_name],
        color=colors[predictor_name],
    )
    axes[1].plot(
        r_annual[predictor_name].sel(iwp=slice(1e-3, 20)).iwp,
        r_annual[predictor_name].sel(iwp=slice(1e-3, 20)),
        label=labels[predictor_name],
        color=colors[predictor_name],
    )
