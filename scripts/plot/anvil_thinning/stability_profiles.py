...
mean_rad_tendency = (ds_top_raw['net_rad_tendency'] + ds_bottom_raw['net_rad_tendency']) / 2
...
t_cold = ds_bottom_raw['t'].sel(hybrid=slice(60, 90)).weighted(ds_bottom_raw['convergence'].sel(hybrid=slice(60, 90))).mean(dim='hybrid')
...
print(f"Difference in temperature at max detrainment: {t_warm.values - t_cold.values:.2f} K")
