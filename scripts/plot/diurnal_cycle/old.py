...
from src.helper_functions import nan_detrend, shift_longitudes, read_ccic_dc, regress_hist_temp_1d
...
# %%
hists['gridsat'] = hist_gridsat.sel(bt=slice(None, 237)).sum('bt')
...
for name in names:
    slopes[name], err[name] = regress_hist_temp_1d(hists_monthly[name], temps[name], hists[name])
...
ccic_region = 'all'
mean_ccic = hists[ccic_region]["hist"].sum('time') / hists[ccic_region]['hist'].sum()
