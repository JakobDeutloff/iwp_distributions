...
def plot_change_diurnal_cycle(slopes, p_values):
    fig, axes = plt.subplots(2, 1, figsize=(5, 3.5), height_ratios=[1, 0.2], sharex=True)
    axes[0].axhline(0, color="black", linewidth=0.5)
    for name in names:
        axes[0].plot(slopes[name].local_time, slopes[name], color=color[name], label=name)
        axes[1].plot(p_values[name].local_time, p_values[name], color=color[name], alpha=0.5)
    for ax in axes:
        ax.set_xlim([0, 24])
...
fig_era5_change.savefig("plots/diurnal_cycle/long_term/era5_1d_change_trend.pdf", bbox_inches="tight")
...
fig.savefig(
    "plots/diurnal_cycle/long_term/linear_trend_1d.pdf", bbox_inches="tight"
)

# %%
fig, ax = plt.subplots()
(cf_ccic_norm["land"] - cf_ccic_norm['land'].mean('time')).plot.pcolormesh(
    "local_time", "time", ax=ax, add_colorbar=True, cmap="RdBu_r", vmin=-0.003, vmax=0.003
)
...
for i, region in enumerate(['all', 'sea']):
    (hists_ccic[region]["hist"].sum('iwp') - hists_ccic[region]["hist"].sum('iwp').mean('local_time')).plot.pcolormesh(
        "local_time", "time", ax=axes[0, i], add_colorbar=False, cmap="RdBu_r", vmin=-1e7, vmax=1e7
    )  
    (cf_ccic_norm[region] - cf_ccic_norm[region].mean('time')).plot.pcolormesh(
        "local_time", "time", ax=axes[1, i], add_colorbar=False, cmap="RdBu_r", vmin=-0.003, vmax=0.003
    )
