...
for region in ['sea', 'all']:
    hist_era5[region] = xr.open_dataset(f"/work/bm1183/m301049/era5/diagnosed/iwp_hist_{region}_2016.nc")
...
# %% interpolate era5
for region in hist_era5.keys():
    hist_era5[region] = hist_era5[region].coarsen(iwp=4, boundary="trim").sum()
...
ax.plot(
    hist_era5['all'].iwp,
    hist_era5['all']['hist'].sel(time="2016").sum(['time', 'local_time']) / hist_era5['all']['size'].sel(time="2016").sum('time'),
    label='ERA5',
    color="#F60019",
    linestyle="--",
    linewidth=2,
...
for region in ['sea', 'land']:
    ax.plot(
        ...
        hist_era5[region]['hist'].sel(time="2016", iwp=slice(0.1, None)).sum(['time', 'iwp']) / hist_era5[region]['size'].sel(time="2016").sum('time'),
        label=f"ERA5 {region}",
        color=colors[region],
    )
