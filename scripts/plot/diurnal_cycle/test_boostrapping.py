...
# %%
with open(
    "/work/bu1562/m301049/diurnal_cycle_dists/ccic_bootstrap_feedback_2d_sample_size_test.pkl",
    "rb",
) as f:
    ds = pickle.load(f)

with open(
    "/work/bu1562/m301049/diurnal_cycle_dists/ccic_bootstrap_feedback_2d_length_test.pkl",
    "rb",
) as f:
...
for i, n in enumerate(n_iterations):
    mean_feedback[i] = (
        ds[n].sum(["iwp", "local_time"]).mean("iteration").mean("repeat_iteration")
    )
    std_feedback[i] = (
        ds[n].sum(["iwp", "local_time"]).mean("iteration").std("repeat_iteration")
    )
...
fig.savefig(
    "plots/diurnal_cycle/publication/ccic_bootstrap_size_test.pdf", bbox_inches="tight"
)
...
for i, w in enumerate(windows):
    mean_feedback_length[i] = (
        ds_length[w]
        .sum(["iwp", "local_time"])
        .mean("iteration")
        .mean("repeat_iteration")
    )
    std_feedback_length[i] = (
        ds_length[w]
        .sum(["iwp", "local_time"])
        .mean("iteration")
        .std("repeat_iteration")
    )
...
ax.fill_between(
    windows,
    mean_feedback_length - std_feedback_length,
    mean_feedback_length + std_feedback_length,
    alpha=0.3,
    color="gray",
)
