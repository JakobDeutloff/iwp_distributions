...
bts = (
    xr.open_mfdataset(
        "/work/bu1562/m301049/GPM_MERGIR/merg_200801011*.nc4", engine="netcdf4"
    )
    .sel(lat=slice(-30, 30))
    .load()
)
iwp = (
    xr.open_mfdataset(
        "/work/bu1562/m301049/ccic/raw/ccic_cpcir_200801011*.zarr", engine="zarr"
    )
...
fig.tight_layout()
fig.savefig(
    "plots/diurnal_cycle/publication/fraction_thin_clouds.pdf", bbox_inches="tight"
)

# %% load hists
hists = {}
hists["ccic"] = xr.open_dataset(
    f"/work/bu1562/m301049/diurnal_cycle_dists/ccic_2d_monthly_all.nc"
)
hists["gpm"] = xr.open_dataset(
    f"/work/bu1562/m301049/diurnal_cycle_dists/gpm_2d_monthly_all.nc"
)
...
for name in ["ccic", "gpm"]:
    area_fractions[name] = hists[name]["hist"].sum(["local_time"]) / hists[name][
        "hist"
    ].sum(["local_time", dim[name]])
# %%
...
# %% make linear fit
res = linregress(
    np.log10(bt_of_iwp.sel(iwp=slice(1, 1e-1))["iwp"].values),
    bt_of_iwp.sel(iwp=slice(1, 1e-1)).mean("time").values,
)
...
ax.set_yticks(
    bt_of_iwp.mean("time").sel(iwp=[10, 1, 0.1], method="nearest").values.round(0)
)
ax.set_xscale("log")
...
with open("/work/bu1562/m301049/diurnal_cycle_dists/bt_iwp_fig_coeffs.pkl", "wb") as f:
...
bt_iwp_ds.to_netcdf("/work/bu1562/m301049/diurnal_cycle_dists/bt_of_iwp.nc")
# %%
