...
# %% open ccic 
hist_ccic = xr.open_mfdataset('/work/bm1183/m301049/ccic_daily_cycle/*/ccic_cpcir_daily_cycle_distribution_2d_*.nc').load()
...
for name in names:
    ...
    slopes[name] = slopes[name] * 100 / hists_monthly[name].mean('time')  # convert to %/K
...
def plot_2d_trend(mean_hist, slopes, p_values, dim):
    ...
    axes[0].scatter(
        local_time_grid[mask], dim_grid[mask],
        color='black', marker='o', s=1, label='p > 0.05'
    )
    ...
    fig.colorbar(im_slope, ax=axes[0], label="Sensitivity / % K$^{-1}$", extend="both", orientation="horizontal")
    fig.colorbar(im_hist, ax=axes[1], label="Normalised Histogram", extend="neither", orientation="horizontal")
    fig.colorbar(im_weighted, ax=axes[2], label="Weighted Sensitivity / K$^{-1}$", extend="both", orientation="horizontal")
    fig.tight_layout()
    return fig, axes

# %% plot slopes ccic
mean_hist = hists['ccic']["hist"].sel(iwp=slice(1e-1, 10)).sum("time") / hists['ccic']["hist"].sel(iwp=slice(1e-1, 10)).sum("time").max()
fig, axes = plot_2d_trend(mean_hist, slopes["ccic"].sel(iwp=slice(1e-1, 10)), p_values["ccic"].sel(iwp=slice(1e-1, 10)), dim='iwp')
fig.savefig('plots/diurnal_cycle/ccic_2d_trend.png', dpi=300)

# %% plot slopes gridsat
mean_hist = hists['gridsat']["hist"].sel(bt=slice(190, 260)).sum("time") / hists['gridsat']["hist"].sel(bt=slice(190, 260)).sum("time").max()
fig, axes = plot_2d_trend(mean_hist, slopes["gridsat"].sel(bt=slice(190, 260)), p_values["gridsat"].sel(bt=slice(190, 260)), dim='bt')
fig.savefig('plots/diurnal_cycle/gridsat_2d_trend.png', dpi=300)

# %% plot slopes gpm
mean_hist = hists['gpm']["hist"].sel(bt=slice(190, 260)).sum("time") / hists['gpm']["hist"].sel(bt=slice(190, 260)).sum("time").max()
fig, axes = plot_2d_trend(mean_hist, slopes["gpm"].sel(bt=slice(190, 260)), p_values["gpm"].sel(bt=slice(190, 260)), dim='bt')
fig.savefig('plots/diurnal_cycle/gpm_2d_trend.png', dpi=300)
                                    
# %% plot mean diurnal cycle 
mean_ccic = hists['ccic'].sel(iwp=slice(1e-1, 10)).sum(["time", 'iwp'])["hist"] / hists['ccic'].sel(iwp=slice(1e-1, 10))['hist'].sum()
mean_gpm = hists['gpm'].sel(bt=slice(190, 230)).sum(["time", "bt"])["hist"] / hists['gpm'].sel(bt=slice(190, 230))['hist'].sum()
mean_gridsat = hists['gridsat'].sel(bt=slice(190, 230)).sum(["time", "bt"])["hist"] / hists['gridsat'].sel(bt=slice(190, 230))['hist'].sum() / 3

# %%
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(mean_ccic["local_time"], mean_ccic, label='CCIC', color=color['ccic'])
ax.plot(mean_gridsat["local_time"], mean_gridsat, label='Gridsat', color=color['gridsat'])
...
# %% 
fig, ax = plt.subplots(figsize=(8, 5))
hists['ccic']['hist'].sum(['local_time', 'time']).plot(ax = ax)
ax2 = ax.twiny()
hists['gpm']['hist'].sum(['local_time', 'time']).plot(ax = ax2, color='orange')
ax.set_xscale('log')
ax2.invert_xaxis()
# %%
