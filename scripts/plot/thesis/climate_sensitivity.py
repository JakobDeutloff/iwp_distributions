...
# %% plot only anvil cloud amount feedback
fig, ax = plt.subplots(figsize=(6, 1))
colors['deutloff'] = "#1dd232"
...
ax.scatter(
    rcemip_estimates["amount"], 0, marker="x", color=colors["rcemip"], label="RCEMIP", alpha=0.7
)
ax.scatter(icon_mean_estimates["amount"], 0, marker="x", color=colors["+2K"], label="ICON Mean", alpha=0.7)
ax.scatter(
    xshield_estimates["amount"], 0, marker="x", color=colors["xshield"], label="XSHIELD", alpha=0.7
)  
for dataset in ["CCIC", "2C-ICE", "DARDAR", "SPARE-ICE"]:
    ax.scatter(
        satellite_estimates["amount"][dataset],
...
ax.set_xlabel(r"$\lambda_{\mathrm{P}}$ / W m$^{-2}$ K$^{-1}$")
fig.savefig("plots/anvil_thinning/talk/feedback_estimates_amount.pdf", bbox_inches="tight")
