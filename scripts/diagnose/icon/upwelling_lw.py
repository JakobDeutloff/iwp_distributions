...
ds_icon_inst = cat.ngc4008a(chunks="auto", zoom=9, time="PT15M").to_dask().pipe(attach_coords)
...
ds = ds.assign(
    {
        "local_time": (
            ('time', 'cell'), local_time.data
            ,
        ),
    })

# %% calculate condensate as measure for clearsky
ds = ds.assign(condensate=(('time', 'cell'), (ds['cli'].sum('level_full') + ds['clw'].sum('level_full')).data))
...
rlut_cs = ds_sel['rlut'].where(ds_sel['condensate'] < ds_sel['condensate'].quantile(0.3)).groupby_bins(ds_sel['local_time'], lt_bins).mean()
rlut_cs = rlut_cs.rename({'local_time_bins': 'local_time'})
rlut_cs['local_time'] = lt_points

# %% save rlut 
rlut_cs.to_netcdf("/work/bm1183/m301049/iwp_dists/publication/fluxes/rlut_cs.nc")