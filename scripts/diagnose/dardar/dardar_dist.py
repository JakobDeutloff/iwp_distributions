...
mask = xr.open_dataset("/work/bu1562/m301049/orcestra/sea_land_mask.nc").pipe(
...
def calc_histogram(year, batch_idx):
    ...
    for day in days:
        ...
        hist, _ = np.histogram(
            ds_selection["iwp"].sel(time=day).values, bins=bins, density=False
        )
...
with ProcessPoolExecutor(max_workers=32) as executor:
    futures = {
        executor.submit(calc_histogram, year, batch_idx): (year, batch_idx)
        for year, batch_idx in year_batch_pairs
    }
