    ...
    ds = ds.assign(
        {
            "local_time": (
                ("time", "lat", "lon"),
                np.tile(local_time.values, (1, ds["lat"].size, 1)),
            ),
        }
    )
    H, _, _ = np.histogram2d(
        ds["local_time"].values.flatten(),
        ds["irwin_cdr"].values.flatten(),
...
hists = xr.Dataset(
    ...
    attrs={
        "description": "2D histogram of Gridsat IR 11m brightness temperature vs local time"
    },
)

# %% save dataset
out_path = "/work/bu1562/m301049/gridsat/coarse"
