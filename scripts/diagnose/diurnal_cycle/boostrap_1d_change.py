...
for ds in datasets:
    for region in regions:
        hists[ds][region] = xr.open_dataset(
            f"/work/bu1562/m301049/diurnal_cycle_dists/{ds}_2d_monthly_{region}_weighted.nc"
        )
...
cf_deseason = {"ccic": {}, "gpm": {}}
cf_deseason_nonnorm = {"ccic": {}, "gpm": {}}
for ds in datasets:
    for region in regions:
...
def calc_regression_bs(seed, dataset="ccic", region="all", len_block=36):
    ...
    slopes, _ = regress_hist_temp_1d(
        cf_deseason[dataset][region].isel(time=time_idx),
        temps_deseason[region],
        cf_norm[dataset][region],
    )
    return slopes
...
for dataset in datasets:
    for region in regions:
        ...
        slopes.to_netcdf(
            f"/work/bu1562/m301049/diurnal_cycle_publication/{dataset}_{region}_bootstrap_slopes_1d.nc"
        )
...
def calc_regression_bs(seed, dataset="ccic", region="all", len_block=36):
    ...
    slopes, _ = regress_hist_temp_1d(
        cf_deseason_nonnorm[dataset][region].isel(time=time_idx),
        temps_deseason[region],
        cf[dataset][region],
    )
    return slopes
...
for dataset in datasets:
    for region in regions:
        ...
        slopes = xr.concat(results, dim="iteration")
        slopes.to_netcdf(
            f"/work/bu1562/m301049/diurnal_cycle_publication/{dataset}_{region}_bootstrap_slopes_1d_nonnorm.nc"
        )
